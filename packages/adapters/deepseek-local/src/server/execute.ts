...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    while (turnCount < maxTurns) {
      ...
      if (!cleanedContent) {
        emit(onLog, { kind: "stderr", ts: ts(), text: "[deepseek] WARNING: Empty response (no text, no tool calls). Treating as completion." });
      ...
      consecutiveTextOnlyTurns++;
      if (consecutiveTextOnlyTurns >= MAX_TEXT_ONLY_TURNS) {
        emit(onLog, { kind: "stderr", ts: ts(), text: `[deepseek] Model responded ${MAX_TEXT_ONLY_TURNS}x with text only (no tool calls, no completion signal). Treating as completed.` });
        messages.push({ role: "assistant", content: contextContent || "" });
        stoppedReason = "completed";
        break;
      }
